...
function Button({ children, disalbed, to, type, onClick }) {
  const base =
  ...
  if (onClick)
    return (
      <button onClick={onClick} disabled={disalbed} className={styles[type]}>
        {children}
      </button>
    );

  return (
    <button disabled={disalbed} className={styles[type]}>
      {children}
    </button>
  );
}
